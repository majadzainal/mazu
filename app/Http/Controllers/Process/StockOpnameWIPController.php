...
class StockOpnameWIPController extends Controller
{
    public $MenuID = '1004';
    public $objStock;
    public $objLogPartStock;
    public $objNumberingForm;
    public $generateType = 'F_STOCK_OPNAME_WIP';
    ...
    public function listStockOpname(){
        ...
        $partWIPOpnameList = StockOpname::where('opname_type_id', 3)
                    ->where('is_active', 1)
        ...
        return view('process.stockOpnameWIPTable', [
            'MenuID' => $this->MenuID,
    ...
    function loadPart($plant_id){
        $stock = DB::table('tm_stock')
            ...
            ->where('tm_part_customer.is_supplier', 1)
        ...
        try {
            ...
            if($stock_opname){
                for ($i=0; $i<count($request->part_customer_id); $i++ ){
                    ...
                    if($item){
                        $this->objStock->plusStockPartCustomer($item->part_customer_id, $item->warehouse_id, $item->deviation, "Stock Opname WIP. (".$stock_opname->stock_opname_number.")");
                ...
                if (count($request->part_customer_id) == $arrsuccess ){
                    DB::commit();
                    return response()->json(['status' => 'Success', 'message' => 'Add stock opname WIP.'], 200);
                } else {
                    DB::rollback();
                    return response()->json(['status' => 'Error', 'message' => 'Add stock opname WIP failled, with a part error.' ], 202);
                }
            }
        ...
        try {
            ...
            if ($opname){
                ...
                foreach ($opname->opname_item as $ls){
                    ...
                    if($item){
                        $this->objStock->minStockPartCustomer($item->part_customer_id, $item->warehouse_id, $item->deviation, "Cancel Stock Opname WIP. (".$opname->stock_opname_number.")");
                    }
                }

                DB::commit();
                return response()->json(['status' => 'Success', 'message' => 'delete stock opaname WIP.'], 200);
            } else {
                DB::rollback();
                return response()->json(['status' => 'Info', 'message' => 'delete stock opaname WIP.'], 200);
            }
