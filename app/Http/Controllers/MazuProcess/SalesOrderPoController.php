...
class SalesOrderPoController extends Controller
{
    public $MenuID = '00502';
    public $objStock;
    ...
    public function listSO(){
        ...
        return view('mazuprocess.salesOrderPoTable', [
            'MenuID'            => $this->MenuID,
    ...
    public function loadSO($start_date, $end_date){
        ...
        $soList = SalesOrder::where('is_active', 1)
                            ...
                            ->with('poCust', 'poCust.customer', 'items', 'items.product', 'items.product.unit', 'paid', 'paid.paidType')
    ...
    public function addSO(Request $request)
    {
        ...
        try {
            ...
            if($request->is_process){
                ...
                $decPaid = floatval($request->dec_paid_fin) + floatval($request->total_paid);
            ...
            if($so){
                ...
                if($request->is_process){
                    $poData = PurchaseOrderCustomer::find($request->po_customer_id);
                    // dd($poData);
                    $sales_order_paid_id_dp = Uuid::uuid4()->toString();
                    SalesOrderPaid::create([
                        'sales_order_paid_id'           => $sales_order_paid_id_dp,
                        'so_id'                         => $so->so_id,
                        'paid_type_id'                  => $poData->paid_type_id,
                        'dec_paid'                      => $poData->dec_paid,
                        'dec_remain'                    => $poData->dec_remain,
                        'is_po_customer'                => 1,
                    ]);
                    ...
                    $soPaid = SalesOrderPaid::create([
                        ...
                        'dec_paid'                      => floatval($request->dec_paid_fin),
                        'dec_remain'                    => $decRemain,
                        'is_po_customer'                => 0,
                    ]);
    ...
    public function printSalesOrder($so_id){
        ...
        $SO = SalesOrder::with('poCust', 'poCust.customer', 'items', 'items.product', 'items.product.unit')
                            ->where('so_id', $so_id)->first();

        if($SO){
            $data = ['data'  => $SO];
            $pdf = App::make('dompdf.wrapper');
            $pdf->loadView('mazuprocess.print.salesOrderPo', $data);
