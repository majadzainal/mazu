...
class SalesOrderExcResellerController extends Controller
{
    ...
    public function listSO(){
        ...
        return view('mazuprocess.salesOrderExcResellerTable', [
            'MenuID'            => $this->MenuID,
    ...
    public function loadSO($start_date, $end_date){
        ...
        $soList = SalesOrder::where('is_active', 1)
                            ...
                            ->with('excReseller', 'items', 'items.product', 'items.product.unit', 'paid', 'paid.paidType')
    ...
    public function addSO(Request $request)
    {
        ...
        try {
            ...
            if ($so && $arrsuccess == count($request->product_id)){
                DB::commit();
                return response()->json(['status' => 'Success', 'message' => 'Add sales order Exclusive Reseller success.'], 200);
            } else {
                DB::rollback();
                return response()->json(['status' => 'Error', 'message' => 'Add sales order Exclusive Reseller failled, with a product error.' ], 202);
            }
        } catch (Exception  $e) {
            DB::rollback();
    ...
    function deleteSO($so_id){
        ...
        try {
            ...
            if ($so){
                ...
                DB::commit();
                return response()->json(['status' => 'Success', 'message' => 'Delete sales order Exclusive Reseller success.'], 200);
            } else {
                DB::rollBack();
                return response()->json(['status' => 'Info', 'message' => 'delete sales order Exclusive Reseller failed.'], 200);
            }
    ...
    public function printSalesOrder($so_id){
        ...
        $SO = SalesOrder::with('excReseller', 'items', 'items.product', 'items.product.unit')
        ...
        if($SO){
            ...
            $pdf->loadView('mazuprocess.print.salesOrderExcReseller', $data);
