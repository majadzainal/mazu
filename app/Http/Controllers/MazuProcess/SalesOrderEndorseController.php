...
class SalesOrderEndorseController extends Controller
{
    ...
    public function listSO(){
        ...
        $store_id = getStoreId();
        $endorseList = Endorse::where('store_id', $store_id)->where('is_active', 1)->get();
        ...
        return view('mazuprocess.salesOrderEndorseTable', [
            'MenuID'            => $this->MenuID,
    ...
    public function loadSO($start_date, $end_date){
        ...
        $soList = SalesOrder::where('is_active', 1)
                            ...
                            ->with('endorse', 'items', 'items.product', 'items.product.unit', 'paid', 'paid.paidType')
    ...
    public function addSO(Request $request)
    {
        ...
        try {
                ...
                for ($i=0; $i<count($request->product_id); $i++ ){
                    ...
                    if($item){
                        if($request->is_process){
                            ...
                            $this->objStock->minStock($item->product_id, $warehouseProduct, $item->qty_order, "Sales Order  endorse ".$so_number);
                            ...
                            foreach ($compositionList as $ls) {
                                if(!$ls->productSupplier->is_service){
                                    ...
                                    $this->objStock->minStockSupplier($ls->product_supplier_id, $ls->productSupplier->stockWarehouse->warehouse_id, $amount_usage, "Sales Order  endorse ".$so_number);
                                }
                            }
                        }
            ...
            if ($so && $arrsuccess == count($request->product_id)){
                DB::commit();
                return response()->json(['status' => 'Success', 'message' => 'Add sales order endorse success.'], 200);
            } else {
                DB::rollback();
    ...
    function updateSO(Request $request){
        ...
        try {
            ...
            if ($so && $arrsuccess == count($request->product_id)){
                DB::commit();
                return response()->json(['status' => 'Success', 'message' => 'Add sales order endorse_id success.'], 200);
            } else {
                DB::rollback();
    ...
    function deleteSO($so_id){
        ...
        try {
            ...
            if ($so){
                ...
                DB::rollBack();
                return response()->json(['status' => 'Info', 'message' => 'delete sales order endorse failed.'], 200);
            }
