...
class SalesOrderMedsosController extends Controller
{
    ...
    public function listSO(){
        ...
        return view('mazuprocess.salesOrderMedsosTable', [
            'MenuID'            => $this->MenuID,
    ...
    public function loadSO($start_date, $end_date){
        ...
        $soList = SalesOrder::where('is_active', 1)
                            ...
                            ->with('items', 'items.product', 'items.product.unit', 'paid', 'paid.paidType')
    ...
    public function addSO(Request $request)
    {
        ...
        try {
            ...
            $so = SalesOrder::create([
                ...
                'medsos_description'            => $request->medsos_description,
                ...
                for ($i=0; $i<count($request->product_id); $i++ ){
                    ...
                    if($item){
                        if($request->is_process){
                            ...
                            $this->objStock->minStock($item->product_id, $warehouseProduct, $item->qty_order, "Sales Order medsos gift away ".$so_number);
                            ...
                            foreach ($compositionList as $ls) {
                                if(!$ls->productSupplier->is_service){
                                    ...
                                    $this->objStock->minStockSupplier($ls->product_supplier_id, $ls->productSupplier->stockWarehouse->warehouse_id, $amount_usage, "Sales Order  medsos gift away ".$so_number);
                                }
                            }
                        }
            ...
            if ($so && $arrsuccess == count($request->product_id)){
                DB::commit();
                return response()->json(['status' => 'Success', 'message' => 'Add sales order medsos gift away success.'], 200);
            } else {
                DB::rollback();
            ...
            if ($so){
                if($so->is_process){
                    foreach ($so->items as $ls) {
                        $this->objStock->plusStock($ls->product_id, $ls->product->stockWarehouse->warehouse_id, $ls->qty_order, "Cancel sales order medsos gift away ".$so->so_number);
                        foreach ($ls->product->composition as $item) {
                            if(!$item->productSupplier->is_service){
                                ...
                                $this->objStock->plusStockSupplier($item->product_supplier_id, $item->productSupplier->stockWarehouse->warehouse_id, $amount_usage, "Cancel sales order medsos gift away ".$so->so_number);
                            }
                        }
                    }
                }
