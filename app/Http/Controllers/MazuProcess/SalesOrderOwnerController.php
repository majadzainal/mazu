...
class SalesOrderOwnerController extends Controller
{
    public $MenuID = '00604';
    public $objStock;
    ...
    public function listSO(){
        ...
        $store_id = getStoreId();
        $ownerList = Owner::where('store_id', $store_id)->where('is_active', 1)->get();
        ...
        return view('mazuprocess.salesOrderOwnerTable', [
            'MenuID'            => $this->MenuID,
            'ownerList'      => $ownerList,
    ...
    public function loadSO($start_date, $end_date){
        ...
        $soList = SalesOrder::where('is_active', 1)
                            ...
                            ->with('owner', 'items', 'items.product', 'items.product.unit', 'paid', 'paid.paidType')
    ...
    public function addSO(Request $request)
    {
        ...
        try {
                ...
                for ($i=0; $i<count($request->product_id); $i++ ){
                    ...
                    if($item){
                        if($request->is_process){
                            ...
                            $this->objStock->minStock($item->product_id, $warehouseProduct, $item->qty_order, "Sales Order  owner ".$so_number);
                            ...
                            foreach ($compositionList as $ls) {
                                if(!$ls->productSupplier->is_service){
                                    ...
                                    $this->objStock->minStockSupplier($ls->product_supplier_id, $ls->productSupplier->stockWarehouse->warehouse_id, $amount_usage, "Sales Order owner ".$so_number);
                                }
                            }
                        }
            ...
            if ($so && $arrsuccess == count($request->product_id)){
                DB::commit();
                return response()->json(['status' => 'Success', 'message' => 'Add sales order owner  success.'], 200);
            } else {
                DB::rollback();
                return response()->json(['status' => 'Error', 'message' => 'Add sales order owner failled, with a product error.' ], 202);
            }
    ...
    function deleteSO($so_id){
        ...
        try {
            ...
            if ($so){
                ...
                DB::commit();
                return response()->json(['status' => 'Success', 'message' => 'Delete sales order owner success.'], 200);
            } else {
                DB::rollBack();
                return response()->json(['status' => 'Info', 'message' => 'delete sales order owner failed.'], 200);
            }
    ...
    public function printSalesOrder($so_id){
        ...
        $SO = SalesOrder::with('owner', 'items', 'items.product', 'items.product.unit')
        ...
        if($SO){
            ...
            $pdf->loadView('mazuprocess.print.salesOrderOwner', $data);
