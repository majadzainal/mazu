            ...
            <div class="row align-items-center">
                <div class="col-md-8">
                    <div class="page-header-title">
                        <h5 class="m-b-10">SALES ORDER OWNER TABLE</h5>
                        <p class="m-b-0">&nbsp;</p>
                    </div>
                </div>
                <div class="col-md-4">
                    <ul class="breadcrumb">
                        ...
                        <li class="breadcrumb-item"><a href="#!">Sales Order Owner</a></li>
                    </ul>
                </div>
            </div>
        </div>
                    ...
                    <div class="row">
                            ...
                            <div class="card">
                                ...
                                <div class="card-block">
                                    <form action="/process/sales-order-special-gift/add" method="post" enctype="multipart/form-data" id="soForm">
                                        ...
                                        <div class="form-group row">
                                            <label class="col-sm-2 col-form-label">Owner <span class="text-danger">*</span></label>
                                            <div class="col-sm-10">
                                                <select name="owner_id" id="owner_id" class="js-example-placeholder col-sm-12" required>
                                                    <option value="">--Select--</option>
                                                    @foreach($ownerList as $ls)
                                                        <option value="{{ $ls->owner_id }}">{{ $ls->owner_name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                        ...
                        <div class="col-sm-12" id="payment">
                            <div class="card">
                                ...
                                <div id="payment_form">
                                    <form action="/process/sales-order-owner-payment/add" method="post" enctype="multipart/form-data" id="paymentForm">
                                        @csrf
...
<script>
    ...
    function loadData(start_date, end_date){
        ...
        var table = $('#soTable').DataTable({
            ...
            "bFilter": true,
            "bInfo": true,
            "ajax": '/process/sales-order-owner/load/'+start_date+'/'+end_date,
            "aoColumns": [
                ...
                {  "mRender": function (data, type, row, num) {
                        var customerStatus = "";
                        customerStatus += row.owner.owner_name;
    ...
    function loadProduct(){
        ...
        var table = $('#productTableList').DataTable({
            ...
            "bFilter": true,
            "bInfo": true,
            "ajax": '/process/sales-order-owner/load-product',
            "aoColumns": [
                {  "mRender": function (data, type, row, num) {
                        return num.row+1;
                    }
    ...
    async function return_value(e, data){
        ...
        var btn = $(e).attr("btn");
        if (btn == "edit"){
            $("#soForm").attr("action", "/process/sales-order-owner/update");
            ...
            $("#owner_id").trigger('change');
            $("#btnCancelPO").hide();
            $("#soForm").attr("action", "/process/sales-order-owner/add");
    ...
    async function addToReceivingTable(product_label){
        if(product_label !== ""){
            var data = await getData('/process/sales-order-owner/get-label/'+product_label);
            if(data != null){
    ...
    function loadDataPayment(so_id){
        ...
        var table = $('#paymentTable').DataTable({
            ...
            "bFilter": true,
            "bInfo": true,
            "ajax": '/process/sales-order-owner-payment/load/'+so_id,
            "aoColumns": [
                {  "mRender": function (data, type, row, num) {
                        return num.row+1;
                    }
