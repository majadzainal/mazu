                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-12" id="input">
                            <div class="card">
                                ...
                                <div class="card-block">
                                    <form action="/process/sales-order-po/add" method="post" enctype="multipart/form-data" id="soForm">
                                        @csrf
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-sm-10">
                                                <select name="po_customer_id" id="po_customer_id" onchange="poCustomerChange(0)" class="js-example-placeholder col-sm-12" required>
                                        ...
                                        <div class="form-group row modal-footer">
                                            <div class="col-sm-12" style="text-align:right;">
                                                <button type="button" class="btn btn-default waves-effect closeForm" onclick="closeForm()">Close</button>
                        ...
                        <div class="col-sm-12" id="payment">
                            <div class="card">
                                ...
                                <div id="payment_form">
                                    <form action="/process/sales-order-po-payment/add" method="post" enctype="multipart/form-data" id="paymentForm">
                                        @csrf
...
<script>
    $(document).ready(function() {
        ...
        btnLoadDataClick();
        // $("#product_label").attr('readonly', true);
        // loadProduct();
        $("#input").hide();
        $("#payment").hide();
    });
    ...
    function loadData(start_date, end_date){
        ...
        var table = $('#soTable').DataTable({
            ...
            "bFilter": true,
            "bInfo": true,
            "ajax": '/process/sales-order-po/load/'+start_date+'/'+end_date,
            "aoColumns": [
                ...
                },
                {
                    ...
                    "mRender": function (data, type, row) {
                        ...
                        @if(isAccess('delete', $MenuID))
                            button += "<button class='btn waves-effect waves-light btn-warning btn-icon' data-confirm='Are you sure|want to delete "+ row.so_number +"("+ row.po_cust.customer.customer_name+") sales order??' data-url='/process/sales-order-po/delete/" + data + "' onClick='deleteInit(this)'>&nbsp;<i class='icofont icofont-trash'></i></button>";
                        @endif
                        return button;
                    }
                },
                ...
                {  "mRender": function (data, type, row, num) {
                        var customerStatus = "";
                        customerStatus += row.po_cust.customer.customer_name;
    ...
    function return_value(e, data){
        ...
        if (btn == "edit"){
            ...
            if(po_customer_id !== ""){
                $("#po_customer_id").val(po_customer_id);
            ...
            // $('#$("#bodyProduct").append(addProduct);').DataTable().destroy();
            $("#bodyProduct").html("");
    ...
    function addToTableProductItem(data){
        ...
        $('#doItemTableList tr').each(function() {
            ...
            if(product_id === data.product_id){
                ...
                var poCustId = $("#po_customer_id").val();
                if(poCustId !== ""){
                    var poList = {!! json_encode($poCustomerList) !!};
                    var poData = poList.find(x => x.po_customer_id == poCustId);
                    if(poData !== null){
                        var productData = poData.items.find(x => x.product_id == data.product_id);
                        var qty = $(this).find(".qty_order_item").val();
                        if(qty >= productData.qty_order){
                            swal('Info', 'Qty order sudah terpenuhi. ', 'info');
                        }else{
                            if(dataLabel.length >= 1){
                ...
                swal('Info', 'Product label ['+data.no_label+'] is diferent product with PO, please scan other product label', 'info');
            }
        });
    ...
    function poCustomerChange(is_edit){
        var poCustId = $("#po_customer_id").val();
        if(poCustId !== "" && is_edit === 0){
            var poList = {!! json_encode($poCustomerList) !!};
            var data = poList.find(x => x.po_customer_id == poCustId);
            justFillToFormByPo(data);
        }
    }

    function justFillToFormByPo(data){
    ...
    async function addToReceivingTablePO(product_label){
        if(product_label !== ""){
    ...
    function loadDataPayment(so_id){
        ...
        var table = $('#paymentTable').DataTable({
            ...
            "bFilter": true,
            "bInfo": true,
            "ajax": '/process/sales-order-po-payment/load/'+so_id,
            "aoColumns": [
                {  "mRender": function (data, type, row, num) {
                        return num.row+1;
                    }
    ...
    function return_print(e, data){
        window.open('/process/sales-order-po-print/'+data.so_id, '_blank');
    }
