            ...
            <div class="row align-items-center">
                <div class="col-md-8">
                    <div class="page-header-title">
                        <h5 class="m-b-10">SALES ORDER OUTLET TABLE</h5>
                        <p class="m-b-0">&nbsp;</p>
                    </div>
                </div>
                <div class="col-md-4">
                    <ul class="breadcrumb">
                        ...
                        <li class="breadcrumb-item"><a href="#!">Sales Order Outlet</a></li>
                    </ul>
                </div>
            </div>
        </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-12" id="input">
                            <div class="card">
                                ...
                                <div class="card-block">
                                    <form action="/process/sales-order-outlet/add" method="post" enctype="multipart/form-data" id="soForm">
                                        @csrf
                                            ...
                                            <div class="col-sm-10">
                                                <select name="outlet_id" id="outlet_id" class="js-example-placeholder col-sm-12" required>
                                                    ...
                                                    @foreach($outletList as $ls)
                                                        <option value="{{ $ls->outlet_id }}">{{ $ls->outlet_name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                        ...
                        <div class="col-sm-12" id="payment">
                            <div class="card">
                                ...
                                <div id="payment_form">
                                    <form action="/process/sales-order-outlet-payment/add" method="post" enctype="multipart/form-data" id="paymentForm">
                                        @csrf
...
<script>
    ...
    function loadData(start_date, end_date){
        ...
        var table = $('#soTable').DataTable({
            ...
            "bFilter": true,
            "bInfo": true,
            "ajax": '/process/sales-order-outlet/load/'+start_date+'/'+end_date,
            "aoColumns": [
                ...
                },
                {
                    ...
                    "mRender": function (data, type, row) {
                        ...
                        @if(isAccess('delete', $MenuID))
                            button += "<button class='btn waves-effect waves-light btn-warning btn-icon' data-confirm='Are you sure|want to delete "+ row.so_number +"("+ row.outlet.outlet_name+") sales order outlet??' data-url='/process/sales-order-outlet/delete/" + data + "' onClick='deleteInit(this)'>&nbsp;<i class='icofont icofont-trash'></i></button>";
                        @endif
                        return button;
                    }
                },
                ...
                {  "mRender": function (data, type, row, num) {
                        var customerStatus = "";
                        customerStatus += row.outlet.outlet_name;
    ...
    function loadProduct(){
        ...
        var table = $('#productTableList').DataTable({
            ...
            "bFilter": true,
            "bInfo": true,
            "ajax": '/process/sales-order-outlet/load-product',
            "aoColumns": [
                {  "mRender": function (data, type, row, num) {
                        return num.row+1;
                    }
    ...
    async function return_value(e, data){
        ...
        var btn = $(e).attr("btn");
        if (btn == "edit"){
            $("#soForm").attr("action", "/process/sales-order-outlet/update");
            ...
            $("#outlet_id").trigger('change');
            $("#btnCancelPO").hide();
            $("#soForm").attr("action", "/process/sales-order-outlet/add");
    ...
    async function addToReceivingTable(product_label){
        if(product_label !== ""){
            var data = await getData('/process/sales-order-outlet/get-label/'+product_label);
            if(data != null){
    ...
    function loadDataPayment(so_id){
        ...
        var table = $('#paymentTable').DataTable({
            ...
            "bFilter": true,
            "bInfo": true,
            "ajax": '/process/sales-order-outlet-payment/load/'+so_id,
            "aoColumns": [
                {  "mRender": function (data, type, row, num) {
                        return num.row+1;
                    }
