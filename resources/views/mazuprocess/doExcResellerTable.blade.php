            ...
            <div class="row align-items-center">
                <div class="col-md-8">
                    <div class="page-header-title">
                        <h5 class="m-b-10">DELIVERY ORDER EXCLUSIVE RESELLER TABLE</h5>
                        <p class="m-b-0">&nbsp;</p>
                    </div>
                </div>
                <div class="col-md-4">
                    <ul class="breadcrumb">
                        ...
                        <li class="breadcrumb-item"><a href="#!">Delivery Order Exclusive Reseller</a></li>
                    </ul>
                </div>
            </div>
        </div>
                    ...
                    <div class="row">
                        <div class="col-sm-12" id="table">
                            <div class="card">
                                <div class="card-header">
                                    <h5>Delivery Order Exclusive Reseller</h5>
                                    <span></span>
                        ...
                        <div class="col-sm-12" id="input">
                            <div class="card">
                                ...
                                <div class="card-block">
                                    <form action="/process/delivery-order-exclusive-reseller/add" method="post" enctype="multipart/form-data" id="doOutletForm">
                                        @csrf
                                        <input type="hidden" name="do_exc_reseller_id" id="do_exc_reseller_id">
                                        <div class="form-group row">
                                            <label class="col-sm-2 col-form-label">Delivery Order Exclusive Reseller Number </label>
                                            <div class="col-sm-5">
                                        ...
                                        <div class="form-group row">
                                            <label class="col-sm-2 col-form-label">Reseller <span class="text-danger">*</span></label>
                                            <div class="col-sm-10">
                                                <select name="exc_reseller_id" id="exc_reseller_id" class="js-example-placeholder col-sm-12" required>
                                                    ...
                                                    @foreach($excResellerList as $ls)
                                                        <option value="{{ $ls->exc_reseller_id }}">{{ $ls->reseller_code." - ".$ls->reseller_name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
...
<script>
    ...
    function loadData(start_date, end_date){
        ...
        var table = $('#newTables').DataTable({
            ...
            "bFilter": true,
            "bInfo": true,
            "ajax": '/process/delivery-order-exclusive-reseller/load/'+start_date+'/'+end_date,
            "aoColumns": [
                ...
                },
                {
                    ...
                    "mRender": function (data, type, row) {
                        ...
                        @if(isAccess('delete', $MenuID))
                            button += "<button class='btn waves-effect waves-light btn-warning btn-icon' data-confirm='Are you sure|want to delete outlet "+ row.exc_reseller.reseller_name +" ??' data-url='/process/delivery-order-exclusive-reseller/delete/" + data + "' onClick='deleteInit(this)'>&nbsp;<i class='icofont icofont-trash'></i></button>";
                        @endif
                        return button;
                    }
                },
                { "data": "do_number" },
                { "data": "exc_reseller.reseller_name" },
                { "data": "description" },
            ]
        });
    }
    ...
    function loadProduct(){
        ...
        var table = $('#productTableList').DataTable({
            ...
            "bFilter": true,
            "bInfo": true,
            "ajax": '/process/delivery-order-exclusive-reseller/load-product',
            "aoColumns": [
                {  "mRender": function (data, type, row, num) {
                        return num.row+1;
                    }
    ...
    async function return_value(e, data){
        ...
        var btn = $(e).attr("btn");
        if (btn == "edit"){
            $("#doOutletForm").attr("action", "/process/delivery-order-exclusive-reseller/update");
            $("#do_exc_reseller_id").val(data.do_exc_reseller_id);
            ...
            $("#btnCancelPO").hide();
            $("#doOutletForm").attr("action", "/process/delivery-order-exclusive-reseller/add");
    ...
    async function addToReceivingTable(product_label){
        if(product_label !== ""){
            var data = await getData('/process/delivery-order-exclusive-reseller/get-label/'+product_label);
            if(data != null){
