            ...
            <div class="row align-items-center">
                <div class="col-md-8">
                    <div class="page-header-title">
                        <h5 class="m-b-10">APPROVER PURCHASE ORDER TABLE</h5>
                        <p class="m-b-0">&nbsp;</p>
                    </div>
                </div>
                <div class="col-md-4">
                    <ul class="breadcrumb">
                        ...
                        <li class="breadcrumb-item"><a href="#!">Approver Purchase Order</a></li>
                    </ul>
                </div>
            </div>
        </div>
                                ...
                                <div class="card-block">
                                    <form action="/process/adjustment-purchase-order/update" method="post" enctype="multipart/form-data" id="purchaseForm">
                                        ...
                                        <div class="form-group row modal-footer">
                                            ...
                                            <div id="btn-action" class="col-sm-12" style="text-align:right;">
                                                ...
                                                <button type="button" class="btn btn-info waves-effect waves-light process" onClick="saveInit('#purchaseForm', 3)">Reject</button>
                                                <button type="button" class="btn btn-primary waves-effect waves-light process" onClick="saveInit('#purchaseForm', 5)">Approve</button>
                                            </div>
                                        </div>
                                    </form>
                                </div>
...
<script>
    ...
    function loadData(month_periode, year_periode){
        ...
        var table = $('#newTables').DataTable({
            "ajax": '/process/approver-purchase-order/load/'+month_periode+'/'+year_periode,
            "aoColumns": [
    ...
    async function return_value(e, data){
        ...
        $("#table").hide();
        $(".process").show();
        if(data.status_process > 5){
            $(".process").hide();
        }
        document.documentElement.scrollTop = 0;
        $("#purchaseForm").attr("action", "/process/approver-purchase-order/update");
        await getData('/process/approver-purchase-order/get/'+data.po_id).then(function(result){
            fillToForm(result);
    ...
    async function addMaterial(data){
        ...
        data.po_items.forEach(function(item) {
            ...
            if(data.additional == 1){
                ...
                material +='<td> <input type="number" name="ng_rate[]" class="ng_rate form-control tright" style="width:90px" value="'+ item.qty_ng_rate +'" onInput="countTotal(this)" readonly></td>';
                material +='<td> <input type="number" name="buffer_stock[]" class="buffer_stock form-control tright" style="width:90px" value="'+ item.buffer_stock +'" onInput="countTotal(this)" readonly></td>';
