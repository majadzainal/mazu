    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <form action="" method="post" enctype="multipart/form-data" id="scheduleForm">
                @csrf
                <input type="hidden" name="opname_schedule_id" id="opname_schedule_id">
...
<script>
    ...
    function loadData(){
        ...
        $('#searchTable').DataTable({
            ...
            "aoColumns": [
                ...
                {
                    "mData": "opname_schedule_id",
                    "mRender": function (data, type, row, num) {
                        var button_open = "<button class='btn-sm btn-danger' data-confirm='Are you sure|want to close opname schedule "+ row.opname_date +" - "+  row.plant.plant_name  +"??' data-url='/process/stock-opname-schedule/close/" + data + "' onClick='closeInit(this)'>open</button>";
                ...
                { "data": "end_datetime" },
                {
                    ...
                    "mRender": function (data, type, row) {
                        ...
                        @if(isAccess('delete', $MenuID))
                            button += "<button class='btn waves-effect waves-light btn-warning btn-icon' data-confirm='Are you sure|want to delete opname schedule "+ row.opname_date +"-"+ row.plant.plant_name +" ??' data-url='/process/stock-opname-schedule/delete/" + data + "' onClick='deleteInit(this)'>&nbsp;<i class='icofont icofont-trash'></i></button>";
                        @endif
                        return button;
                    }
                }
    ...
    function return_value(e, data){
        ...
        if (btn == "edit"){
            ...
            $("#opname_schedule_id").val(data.opname_schedule_id);
