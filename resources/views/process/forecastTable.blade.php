            ...
            <div class="row align-items-center">
                <div class="col-md-8">
                    <div class="page-header-title">
                        <h5 class="m-b-10">FORECAST TABLE</h5>
                        <p class="m-b-0">&nbsp;</p>
                    </div>
                </div>
                <div class="col-md-4">
                ...
                <div class="page-body">
                    <div class="row">
                        ...
                        <div class="col-sm-12" id="input">
                            <div class="card">
                                <div class="card-header">
                                    <h5>Detail forecast by sales order</h5>
                                    <span></span>
                                    <div class="card-header-right">
                                        <ul class="list-unstyled card-option">
                                            <li><i class="fa fa fa-wrench open-card-option"></i></li>
                                ...
                                <div class="card-block">
                                    ...
                                    <div class="form-group row modal-footer">
                                        <div class="col-sm-12" style="text-align:right;">
                                            <button type="button" class="btn btn-danger waves-effect closeForm">Close</button>
                                        </div>
                                    </div>
                                    <div class="form-group" style="text-align:left;">
                                        <hr>
                                        <label class="col-sm-12 col-form-label"><h4> Part Supplier Forecast</h4></label>
                                    </div>
                                    <div class="table-responsive">
                                        <table class="table table-sm">
                                            <thead>
...
<script>
    ...
    async function return_value(e, data){
        ...
        await getData('/process/forecast/get/'+data.sales_order_id).then(function(result){
    ...
    function justFillToTablePartSupplier(items){
        ...
        if(items.length >= 1){
            items.forEach(function(item) {
                periodeBefore = isFirst ? item.next_month : periodeBefore;
                var periodeAfter = item.next_month;

                if(periodeBefore !== periodeAfter){
                    addSummarySupplier(total_price, periodeBefore);
                    periodeBefore = item.next_month;
                    total_price = 0;
                    isFirst = true;
                }
    ...
    function addSummarySupplier(total_price, month_periode){
        var part = '<tr>';
        part += '<td></td>';
        part += '<td></td>';
        part += '<td></td>';
        part += '<td></td>';
        part += '<td class="text-right"><label class="col-form-label"><strong>Total M+'+month_periode+': </strong></label></td>';
        part += '<td>';
    ...
    function addPartSupplier(item){
        var part = '<tr>';
        part += '<td>';
        part += '<input type="text" value="'+ item.part_number +' - '+ item.part_name +'" name="part_supplier_id[]" class="part_supplier_id form-control col-sm-12" readonly>';
        part += '</td>';
        part += '<td>';
        part += '<input type="text" value="M+'+ item.next_month +'" name="unit[]" class="unit form-control col-sm-12" readonly>';
        part += '</td>';
        part += '<td>';
        part += '<input type="text" value="'+ numbering(Number(item.qty).toFixed(8)) +'" name="qty[]" class="qty form-control col-sm-12" readonly>';
        part += '</td>';
        part += '<td>';
        part += '<input type="text" value="'+ item.unit_name +'" name="unit[]" class="unit form-control col-sm-12" readonly>';
        part += '</td>';
        part += '<td>';
        part += '<input type="text" value="'+ numbering(item.price) +'" name="price[]" class="price text-right form-control col-sm-12" readonly>';
        part += '</td>';
