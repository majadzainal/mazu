...
function loadSelect2(){
    ...
    $('.select-style').select2({
        placeholder: "--select--",
        templateResult: formatDesignResult,
        templateSelection: formatDesignSelection
    });

}

function formatDesignResult(item) {
    var selectionText = item.text.split("+");
    var htmlText = '<span>';
        if (selectionText.length > 0){
            htmlText += selectionText[0];
            for(var i=1; i<selectionText.length; i++){
                htmlText += '</br>'+selectionText[i];
            }
        } else {
            htmlText += item.text ;
        }
    ...
    var $returnString = $(htmlText);
    return $returnString;
};

function formatDesignSelection(item) {
    var selectionText = item.text.split("+");
    var $returnString = "";
    
    if (selectionText.length > 0)
        $returnString = selectionText[0];
    else
